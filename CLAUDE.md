...
- **Model:** Gemma 4 E2B (`gemma-4-E2B-it.litertlm`, ~2.5 GB). Not bundled — fetched on first `warmUp()`. The download (`ModelDownloader`) is **resumable + retrying**: it keeps a `<model>.part` file across failures, resumes with an HTTP `Range` request, and retries with backoff, so a transient stall doesn't restart the multi-GB transfer (see `DECISIONS.md` 0011).
- **Model location (single copy, reuse across reinstalls):** the model lives in **one** place — app-specific external storage (`getExternalFilesDir(null)/llm/`) when mounted, falling back to internal `filesDir/llm/` otherwise. `warmUp()` downloads into that location and `loadModel()` loads directly from it; there is **no** separate internal copy or import/export step. The external dir is `adb push`-able, so re-seeding across clean installs is a fast local push rather than a 20-min download. It is wiped on uninstall. External path: `/sdcard/Android/data/org.dhis2.multiplatformmobileplayground/files/llm/`. To reuse the model across clean installs/devices: `adb pull` it once, then `adb push` it back into that dir and launch — the app loads it in place.
